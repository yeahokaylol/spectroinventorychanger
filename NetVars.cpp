...
void CNetVarManager::Initialize()
{
	...
	ClientClass* clientClass = Interfaces.pClient->GetAllClasses();
	if (!clientClass)
		return;
...
int CNetVarManager::Get_Prop(RecvTable* recvTable, const char* propName, RecvProp** prop)
{
	...
	for (int i = 0; i < recvTable->m_nProps; ++i)
	{
		...
		if (stricmp((const char*)recvProp->m_pVarName, propName))
			continue;
...
void CNetVarManager::DumpNetvars()
{
	...
	m_file << ("NetVar Dumper by Omniz \n\n\n\n");

	for (ClientClass* pClass = Interfaces.pClient->GetAllClasses(); pClass != NULL; pClass = pClass->m_pNext)
	{
